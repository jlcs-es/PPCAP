...
main()
{
  ...
  // fila_i_c = fila_i_a * b = b' * fila_i_a
  // CblasTrans => nos transpone b  // CblasNoTrans => ya se lo damos transpuesto
  for(i=0;i<fa;i++)
  {
    cblas_dgemv (CblasRowMajor, CblasNoTrans, cb, fb, alpha, b, fb, &a[i*lda], 1, beta, &c[i*ldc], 1);
  }
